...
pub fn process_deposit_dynamic_pool(args: &Args, sub_args: &DepositDynamicAmmArgs) {
    ...
    let client = args.to_rpc_args().rpc_client();
    ...
    let a_vault_state: Vault = args.to_rpc_args().get_account(pool_state.a_vault).unwrap();
    let b_vault_state: Vault = args.to_rpc_args().get_account(pool_state.b_vault).unwrap();
...
fn get_pool_token_amount(
    ...
    let pool_lp_mint: Mint = args.to_rpc_args().get_account(pool_state.lp_mint)?;
